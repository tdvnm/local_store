<script lang="ts">
  ...
  const oos = $derived(!product.isAvailable);
  const needsConfirmation = $derived(product.inventoryType === 2);
  ...
  const displayPrice = $derived(product.pricePaise / 100);
</script>
...
<div class="card p-3 {oos ? 'opacity-50' : ''} relative">
  ...
  {#if product.imageUrl}
    <div class="h-20 flex items-center justify-center">
      <img src={product.imageUrl} alt={product.name} class="h-full w-full object-contain rounded" />
    </div>
  {:else}
  ...
  <div class="flex items-center justify-between mt-1">
    <span class="text-price font-bold">&#8377;{displayPrice}</span>
  </div>
